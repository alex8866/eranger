    ...
    def _get_left_part(self, bar):
        ...
        if self.fm.var == '':
            ...
            if self.fm.thisfile is not None and \
                ...
                bar.add('+++ ', 'hostname', clr, fixed=True)
                #bar.add(self.fm.files[0], 'hostname', clr, fixed=True)
                #bar.add('/', 'hostname', clr, fixed=True)
                bar.add(str(self.fm.thisfile).replace('%s/%s/' % (self.fm.tempdir, self.fm.var), '') , 'hostname', clr, fixed=True)
                bar.add(' --- ', 'link', clr, fixed=True)
                #bar.add(self.fm.files[1], 'hostname', clr, fixed=True)
                bar.add(str(self.fm.thisfile).replace('%s/%s/' % (self.fm.tempdir, self.fm.var), '') , 'link', clr, fixed=True)
